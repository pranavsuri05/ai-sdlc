...
Phase 8B-3: the BRD hop, the HLD + Initial-User-Story hop, and the LLD hop are
modelled. This function never writes, never generates, never finalizes, and
never invokes the graph. It exists so the UI / callers can decide what the next
runnable step is without side effects.
"""
...
NEXT_NONE = None                     # LLD is final; 8B-3 has no further graph steps
...
def sdlc_status(
    ...
    *,
    ...
    """Return a plain dict describing BRD / HLD / User-Story / LLD state and the
    next runnable step.
    ...
    source). All getters used here are read-only and make no Gemini call.
    """
